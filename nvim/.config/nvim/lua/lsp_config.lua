...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(event)
    vim.keymap.set("n", "gd", vim.lsp.buf.definition,
      { buffer = true, desc = "LSP: Go to definition" })
    vim.keymap.set("n", "grt", vim.lsp.buf.type_definition,
      { buffer = true, desc = "LSP: Go to type definition" })
    vim.keymap.set("n", "gD", vim.lsp.buf.declaration,
      { buffer = true, desc = "LSP: Go to declaration" })
    vim.keymap.set("n", "gri", vim.lsp.buf.implementation,
      { buffer = true, desc = "LSP: Go to implementation" })
    vim.keymap.set("n", "grr", vim.lsp.buf.references,
      { buffer = true, desc = "LSP: List references" })
    vim.keymap.set("n", "K",
    ...
    vim.keymap.set("i", "<c-k>",
      ...
      { buffer = true, desc = "LSP: Signature Help (insert mode)" })
    vim.keymap.set("n", "gK",
      function() vim.lsp.buf.signature_help({ border = "rounded" }) end,
      { buffer = true, desc = "LSP: Signature Help" })
    vim.keymap.set("n", "grn", vim.lsp.buf.rename,
      { buffer = true, desc = "LSP: Rename Symbol" })
    vim.keymap.set("n", "gra", vim.lsp.buf.code_action,
      { buffer = true, desc = "LSP: Code Actions" })
    ...
    vim.keymap.set("n", "[d",
      function() vim.diagnostic.jump({ count = -1, float = true }) end,
      { buffer = true, desc = "Diagnostics: Previous" })
